...
<div class="col-md-9 well admin-content" id="home">
    ...
    if (isset($_POST["consultar"])) {
        ...
        if (($_POST["categoria"] == " ") && (empty($_POST["criterio"]))) {
            ...
            echo" </table></div>";
            echo "<p align='center'><a href = 'consultarCriteriosAvaliacao.php'>Voltar</a><p/>";
            mysqli_close($db);
            return 0;
        }
        if (($_POST["categoria"] != " ") && (!empty($_POST["criterio"]))) {

            $query = ("select criterio_avaliacao,id_criterio from criterio_avaliacao where id_categoria = '" . $_POST["categoria"] . "' and criterio_avaliacao = '" . $_POST["criterio"] . "'");
            $res = mysqli_query($db, $query);
        ...
        if (($_POST["categoria"] == " ") && (!empty($_POST["criterio"]))) {
            ...
            if ($consulta = mysqli_fetch_array($res)) {
                ...
                echo "<p> Este critério de avaliação não existe!!</p>";
            }
            echo" </table></div>";
            echo "<p align='center'><a href = 'consultarCriteriosAvaliacao.php'>Voltar</a><p/>";
            mysqli_close($db);
        ...
        if (($_POST["categoria"] != " ") && (empty($_POST["criterio"]))) {
            ...
            $res = mysqli_query($db, $query);
            $consulta1 = mysqli_fetch_array($res);
            $query = ("select categoria.categoria_projeto,criterio_avaliacao.id_criterio,criterio_avaliacao.criterio_avaliacao from criterio_avaliacao inner join  categoria on criterio_avaliacao.id_categoria = categoria.id_categoria and categoria.id_categoria = '" . $_POST["categoria"] . "'");
            $res = mysqli_query($db, $query);
            echo "<div class='panel panel-default'>
                                    <table class='table'> <tr>
                                  <th width='200'>Criterios de Avaliação</th>
                                    </tr> ";
